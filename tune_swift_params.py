...
def run_tuning_for_ratio(
    ...
    output_path = args.output_pattern.format(
        task_name=args.task_name, dataset_name=args.dataset_name, ratio=ratio_str
    )
    study_name = args.study_name_pattern.format(
        task_name=args.task_name, dataset_name=args.dataset_name, ratio=ratio_str
    )
    ...
    def save_best_callback(study: optuna.study.Study, trial: optuna.trial.FrozenTrial):
        if study.best_trial.number == trial.number:
            ...
            save_optimal_config(current_config, mapped_params, output_path)
...
def main():
    ...
    parser.add_argument("-d", "--dataset", type=str, required=True, help="数据集文件路径 (用于训练+验证)")
    ...
    parser.add_argument(
        "--output-pattern",
        type=str,
        default="configs/{task_name}/{dataset_name}/swift_best_ar_{ratio}.toml",
        help="输出文件路径模式",
    )
    parser.add_argument(
        "--study-name-pattern",
        type=str,
        default="{task_name}-{dataset_name}-ar{ratio}",
        help="Optuna 研究名称模式",
    )
