...
class WaveletPatcher(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        # SWT
        xn = x.reshape(B * C, T)
        # list[(B * C, T)], len=L+1, [A_L, D_L, D_{L-1}, …, D_1]
        coeffs = swt(xn, wavelet=self.wavelet, level=self.level, axis=-1)
        coeff_stack = torch.stack(coeffs, dim=1)  # (B * C, L+1, T)
        coeff_stack = coeff_stack.reshape(B, C, self.level + 1, T)  # (B, C, L+1, T)

        # 原始小波系数 (B, C*(L+1), T)
        original_coeffs = coeff_stack.reshape(B, C * (self.level + 1), T)

        # -------- patch along time --------
        # (B, C, L+1, T) ➜ unfold ➜ (B, C, L+1, P, p)
        ...
        B, P, C, Lp1, p = coeff_stack.shape

        # -------- final output --------
        z_out = coeff_stack.reshape(B * P, C * Lp1, p)  # (B*P, (L+1)*C, p)
        return original_coeffs, z_out
...
class InverseWaveletPatcher(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        """
        B, expanded_C, T = x.shape
        C = expanded_C // (self.level + 1)  # 计算原始通道数

        # 将扩展通道维度分解为 (原始通道数, 小波系数层数)
        # (B, (L+1)*C, T) -> (B, C, L+1, T)
        x = x.reshape(B, C, self.level + 1, T)
        ...
        # 恢复原始的 batch 和通道维度
        # (B*C, T) -> (B, C, T)
        reconstructed_signal = reconstructed_signal.reshape(B, C, T)
